...
public class OVRLint : EditorWindow
{
    ...
    void OnGUI()
    {
        ...
        mShowRecordsStaticCommon = EditorGUILayout.BeginFoldoutHeaderGroup(mShowRecordsStaticCommon,
            $"Common Issues ({mRecordsStaticCommon.Count})", EditorStyles.foldoutHeader);
        if (mShowRecordsStaticCommon)
            DisplayRecords(mRecordsStaticCommon);
        EditorGUILayout.EndFoldoutHeaderGroup();

#if UNITY_ANDROID
        mShowRecordsStaticAndroid =
            EditorGUILayout.BeginFoldoutHeaderGroup(mShowRecordsStaticAndroid,
                $"Quest Issues ({mRecordsStaticAndroid.Count})", EditorStyles.foldoutHeader);
        ...
        {
            mShowRecordsRuntimeCommon = EditorGUILayout.BeginFoldoutHeaderGroup(mShowRecordsRuntimeCommon,
                $"Common Runtime Issues ({mRecordsRuntimeCommon.Count})", EditorStyles.foldoutHeader);
            if (mShowRecordsRuntimeCommon)
                DisplayRecords(mRecordsRuntimeCommon);
            EditorGUILayout.EndFoldoutHeaderGroup();

#if UNITY_ANDROID
            mShowRecordsRuntimeAndroid =
                EditorGUILayout.BeginFoldoutHeaderGroup(mShowRecordsRuntimeAndroid,
                    $"Quest Runtime Issues ({mRecordsRuntimeAndroid.Count})", EditorStyles.foldoutHeader);
            ...
            EditorGUILayout.BeginFoldoutHeaderGroup(false,
                "Quest Runtime Issues (disabled: Build Target is not Android)");
            EditorGUILayout.EndFoldoutHeaderGroup();
            EditorGUI.EndDisabledGroup();
#endif
        }
    ...
    void DisplayRecords(List<FixRecord> records)
    {
        for (int x = 0; x < records.Count; x++)
        {
            ...
            while (x + siblingRecordCount + 1 < records.Count &&
                   records[x + siblingRecordCount + 1].category.Equals(record.category))
                ++siblingRecordCount;

            EditorGUILayout.BeginHorizontal(record.complete
                ? mFixCompleteStyle
                : mFixIncompleteStyle); //2-column wrapper for record
            EditorGUILayout.BeginVertical(GUILayout.Width(20)); //column 1: icon
            EditorGUILayout.LabelField(
                EditorGUIUtility.IconContent(record.complete ? "d_Progress" : "console.warnicon"), GUILayout.Width(20));
    ...
    public static int RunCheck()
    {
        ...
        return mRecordsStaticCommon.Count + mRecordsStaticAndroid.Count + mRecordsRuntimeCommon.Count +
               mRecordsRuntimeAndroid.Count;
    }
    ...
    static void AddFix(eRecordType recordType, string category, string message, FixMethodDelegate method,
        UnityEngine.Object target, bool editModeRequired, params string[] buttons)
    {
        AddFix(recordType, 0 /*sortOrder*/, category, message, method, target, editModeRequired, buttons);
    }

    static void AddFix(eRecordType recordType, int sortOrder, string category, string message, FixMethodDelegate method,
        UnityEngine.Object target, bool editModeRequired, params string[] buttons)
    {
        ...
        switch (recordType)
        {
            case eRecordType.StaticCommon:
                mRecordsStaticCommon.Add(fixRecord);
                break;
            case eRecordType.StaticAndroid:
                mRecordsStaticAndroid.Add(fixRecord);
                break;
            case eRecordType.RuntimeCommon:
                mRecordsRuntimeCommon.Add(fixRecord);
                break;
            case eRecordType.RuntimeAndroid:
                mRecordsRuntimeAndroid.Add(fixRecord);
                break;
        }
    }
    ...
    static void CheckStaticCommonIssues()
    {
        ...
        {
            AddFix(eRecordType.StaticCommon, "General", OVRManager.UnityAlphaOrBetaVersionWarningMessage, null, null,
                false);
        }
        ...
        if (!OVRPluginInfo.IsOVRPluginOpenXRActivated() || OVRPluginInfo.IsOVRPluginUnityProvidedActivated())
        {
            AddFix(eRecordType.StaticCommon, -9999, "Set OVRPlugin to Oculus Utilities-provided (OpenXR backend)",
                "Oculus recommends using OpenXR plugin provided with its Oculus Utilities package.\nYou can enable OpenXR backend for Oculus through the 'Oculus -> Tools -> OVR Utilities Plugin' menu.",
                null, null, false);
        }
        ...
        {
            AddFix(eRecordType.StaticCommon, "Optimize Display Buffer Format",
                "We recommend to enable use32BitDisplayBuffer.",
                delegate(UnityEngine.Object obj, bool last, int selected)
                {
...
#pragma warning restore 618
        ...
        for (int i = 0; i < lights.Length; ++i)
        {
            ...
            {
                AddFix(eRecordType.StaticCommon, "Unbaked Lights",
                    "The following lights in the scene are marked as Baked, but they don't have up to date lightmap data. Generate the lightmap data, or set it to auto-generate, in Window->Lighting->Settings.",
                    null, lights[i], false, null);
            }
        ...
        for (int i = 0; i < renderers.Length; ++i)
        {
            ...
            {
                AddFix(eRecordType.StaticCommon, "Instanced Materials",
                    "Please avoid instanced materials on renderers.", null, renderers[i], false);
            }
        }
    ...
    static void CheckStaticAndroidIssues()
    {
            ...
            PlayerSettings.Android.targetArchitectures != AndroidArchitecture.ARM64)
        {
            ...
            AddFix(eRecordType.StaticAndroid, "Set Target Architecture to ARM64",
                "32-bit Quest apps are no longer being accepted on the Oculus Store.",
                delegate(UnityEngine.Object obj, bool last, int selected)
                {
                    PlayerSettings.Android.targetArchitectures = AndroidArchitecture.ARM64;
                }, null, false, "Fix");
        ...
        {
            AddFix(eRecordType.StaticAndroid, "Set Min Android API Level",
                "Oculus Quest recommend setting minumum API level to " + (int)recommendedAndroidMinSdkVersion,
                delegate(UnityEngine.Object obj, bool last, int selected)
                {
        ...
        if (OVRDeviceSelector.isTargetDeviceQuestFamily &&
            PlayerSettings.Android.targetSdkVersion < recommendedAndroidMinSdkVersion &&
            PlayerSettings.Android.targetSdkVersion != AndroidSdkVersions.AndroidApiLevelAuto)
        {
            AddFix(eRecordType.StaticAndroid, "Set Android Target SDK Level",
                "Oculus Quest apps recommend setting target API level to " +
                (int)requiredAndroidTargetSdkVersion,
                delegate(UnityEngine.Object obj, bool last, int selected)
                {
                    PlayerSettings.Android.targetSdkVersion = AndroidSdkVersions.AndroidApiLevelAuto;
                }, null, false, "Fix");
        ...
        {
            AddFix(eRecordType.StaticAndroid, "Disable Android TV Compatibility",
                "Apps with Android TV Compatibility enabled are not accepted by the Oculus Store.",
                delegate(UnityEngine.Object obj, bool last, int selected)
                {
        ...
        {
            AddFix(eRecordType.StaticAndroid, "Optimize GPU Skinning",
                "If you are CPU-bound, consider using GPU skinning.",
                delegate(UnityEngine.Object obj, bool last, int selected) { PlayerSettings.gpuSkinning = true; }, null,
                false, "Fix");
        }
        ...
        {
            AddFix(eRecordType.StaticAndroid, "Set Color Space to Linear",
                "Oculus Utilities Plugin with OpenXR only supports linear lighting.",
                delegate(UnityEngine.Object obj, bool last, int selected)
                {
        ...
        {
            AddFix(eRecordType.StaticAndroid, "Symmetric Projection Optimization",
        ...
        {
            AddFix(eRecordType.StaticAndroid, "Optimize Clearing",
                "For GPU performance, please don't use Unity's built-in Skybox.",
                delegate(UnityEngine.Object obj, bool last, int selected) { RenderSettings.skybox = null; }, null,
                false, "Clear Skybox");
        }
        ...
        for (int i = 0; i < materials.Length; ++i)
        {
            ...
            {
                AddFix(eRecordType.StaticAndroid, "Optimize Specular Material",
                    "For GPU performance, please don't use specular shader on materials.",
                    delegate(UnityEngine.Object obj, bool last, int selected)
                    {
                        Material thisMaterial = (Material)obj;
            ...
            {
                AddFix(eRecordType.StaticAndroid, "Material Passes", "Please use 2 or fewer passes in materials.", null,
                    materials[i], false);
            }
        }
        ...
        {
            AddFix(eRecordType.StaticAndroid, "Optimize Scripting Backend", "For CPU performance, please use IL2CPP.",
                delegate(UnityEngine.Object obj, bool last, int selected)
                {
                    PlayerSettings.SetScriptingBackend(UnityEditor.BuildTargetGroup.Android,
        ...
        if (effectBaseType != null)
        {
            for (int i = 0; i < monoBehaviours.Length; ++i)
            {
                ...
                {
                    AddFix(eRecordType.StaticAndroid, "Image Effects", "Please don't use image effects.", null,
                        monoBehaviours[i], false);
                }
            }
        }
        ...
        for (int i = 0; i < textures.Length; ++i)
        {
            ...
            {
                AddFix(eRecordType.StaticAndroid, "Optimize Texture Filtering",
                    "For GPU performance, please generate mipmaps or disable trilinear filtering for textures.",
                    delegate(UnityEngine.Object obj, bool last, int selected)
                    {
    ...
    static void CheckRuntimeAndroidIssues()
    {
        ...
        {
            AddFix(eRecordType.RuntimeAndroid, "Graphics Memory",
                "Please use less than 1GB of vertex and texture memory.", null, null, false);
        }
        ...
        {
            AddFix(eRecordType.RuntimeAndroid, "Triangles and Verts",
                "Please use less than 100000 triangles or vertices.", null, null, false);
        }
