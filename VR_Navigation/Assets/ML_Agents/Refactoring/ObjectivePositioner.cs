...
public class ObjectivePositioner : MonoBehaviour
{
    ...
    private List<RLAgent> monitoredAgents = new List<RLAgent>();
    ...
    void Start()
    {
        ...
        {
            currentEnvironment = GetComponentInParent<Environment>();
            if (currentEnvironment == null)
            {
        ...
        var agents = FindObjectsOfType<RLAgent>();
        ...
        if (monitoredAgents.Count == 0)
        {
            ...
            var envs = FindObjectsOfType<Environment>();
    ...
    private void RegisterAgent(RLAgent agent)
    {
        if (!monitoredAgents.Contains(agent))
        {
    ...
    private void OnAgentTerminated(float reward, Environment env)
    {
    ...
    public void RegisterExternalAgent(RLAgent agent)
    {
        RegisterAgent(agent);
    }
